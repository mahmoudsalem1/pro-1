			...
			<div class="card">
				...
				<div class="card-body collapse in">
					<div class="card-block card-dashboard">
							...
							<div role="tabpanel" class="tab-pane fade {{ $key == 0 ? 'active in' : '' }}" id="{{ $lang->code  }}" aria-labelledby="{{ $lang->code }}-tab" aria-expanded="{{ $key == 0 ? 'true' : 'false' }}">
								...
								<table class="table table-striped table-bordered">
									...
									<tr>
										...
										<td>{{ $slider->langWith($lang->code)->title }}</td>
									</tr>
									@if($slider->image != null)
									<tr>
										<th>{{ trans('admin.image', [], $lang->code)  }}</th>
										<td><img style="max-width: 150px;max-height:150px;" src="{{ $slider->image_path }}"></td>
									</tr>
									@endif
									<tr>
										<th>{!! trans('admin.status', [], $lang->code) !!}</th>
										<td>{!! getStatus($slider->show, $lang->code) !!}</td>
									</tr>
						...
						<a href="{{ route('slider.index') }}" class="btn btn-danger">{{ trans('admin.back') }}</a>
					</div>
				</div>
